...
<head>
		...
		$('#addBtn').click(function(){	
			$('#addBoard').submit(function(event){
				...
				if($.trim($('#title').val()).length<1) {
					...
					return false;	
				} else if($.trim($('#contents').val()).length<1) {      //이제 이 조건이 안 먹는다
    ...
    <body>
        ...
        <div class="container">
        <h1>게시글 입력</h1>
            <form action="/board_action.php?mode=insert" method="post" id="addBoard" enctype="multipart/form-data" onsubmit="submitContents()">
